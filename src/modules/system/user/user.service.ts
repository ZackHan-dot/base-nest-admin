...
export class UserService {
    ...
    async list(
        ...
        if (roleId && !reverse) {
            queryBuilde
                .innerJoin('user.roles', 'role', 'role.roleId = :roleId', {
                    roleId,
                })
    ...
    async updateUser(reqUpdateUserDto: ReqUpdateUserDto) {
        ...
        const posts = await this.postService.listByIdArr(
            reqUpdateUserDto.postIds
        );
        const roles = await this.roleService.listByIdArr(
            reqUpdateUserDto.roleIds
        );
        ...
        if (
            await this.redis.get(
                `${USER_VERSION_KEY}:${reqUpdateUserDto.userId}`
            )
        ) {
